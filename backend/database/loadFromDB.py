...
class DB_Data(BaseConfig):
        ...
        if Location is None:
            ...
            for i in tqdm.tqdm(range(len(_DF))):
                ...
                if str(dfi[self.CommentInfo_time].iloc[0]).split(" ")[0] not in self.dateRange[
                    ...
                    self.dateRange[int(dfi[self.CommentInfo_IDIndex].iloc[0])].append(
                        date)
                try:
                    try:
                        UID = int(
                                BilibiliUserInfo[BilibiliUserInfo[self.UserInfo_UserName] == str(dfi[self.CommentInfo_UserName].iloc[0])][self.UserInfo_ID].iloc[0])
                    except:
                        UID = -1
                    self.data.append({
    ...
    def readWangyiNews(self, Location=None, ID_Index=1, length=-1):
        if Location is None:
            ...
            WangyiUserInfo = pd.read_excel(self.WangyiUserData)
            self.data = []
            cnt = 0
            for i in tqdm.tqdm(range(len(_DF))):
                dfi = _DF.iloc[i:i + 1]
                if dfi[self.NewsInfo_platform].iloc[0] != self.platform:
                    continue
                if int(dfi[self.ID_Index].iloc[0]) not in self.dateRange:
                    self.dateRange[int(dfi[self.ID_Index].iloc[0])] = []
                if str(dfi[self.NewsInfo_Time].iloc[0]).split(" ")[0] not in self.dateRange[int(dfi[self.ID_Index].iloc[0])]:
                    date = str(dfi[self.NewsInfo_Time].iloc[0]).split(" ")[0]
                    self.dateRange[int(dfi[self.ID_Index].iloc[0])].append(date)
                self.data.append({
    ...
    def readZhihuComment(self, Location=None, ID_Index=1, length=-1):
        if Location is None:
            ...
            ZhihuUserInfo = pd.read_excel(self.ZhihuUserData)
            self.data = []
            cnt = 0
            for i in tqdm.tqdm(range(len(_DF))):
                dfi = _DF.iloc[i:i + 1]
                ...
                try:
                    UID = int(
                        ZhihuUserInfo[ZhihuUserInfo[self.UserInfo_UserName] == dfi[self.CommentInfo_UserName].iloc[0]][
                            self.UserInfo_ID].iloc[0])
                except:
                    UID = -1
                self.data.append({
